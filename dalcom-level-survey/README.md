...
`npm run verify` 는 대표 케이스 15개와 **전체 경우의 수 2,048가지**를 모두 검사합니다.
...
제목 → 부제 → `약 1분 · 총 10문항` → `Start!` 버튼

교재 표지 두 권은 현재 빼 둔 상태입니다. 되돌리려면 `StartScreen.tsx` 위쪽 주석을
참고하세요. 표지 파일(`cover-level1.jpg`, `cover-level2.jpg`)은 결과 화면에서도
쓰이므로 `public/images/` 에 그대로 있습니다.
